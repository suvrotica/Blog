	...
	<div class="y-axis">
	  <svg width="40" height={$heightStore}>
		<line
		  ...
		  stroke-width="2"
		/>
		<text x="10" y={$yScale.range()[1]/2 - 50} text-anchor="middle" dominant-baseline="middle" transform="rotate(-90, 10, {$yScale.range()[1] / 2})" fill="currentColor">Y-axis</text>
	  </svg>
	</div>
  ...
  <div class="x-axis">
	<svg width={$widthStore} height="40">
	  <line
		...
		stroke-width="2"
	  />
	  <text x={$widthStore / 2} y="30" text-anchor="middle" fill="currentColor">X-axis</text>
	</svg>
  </div>
